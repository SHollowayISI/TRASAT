...
scenario.multi.n_re = 18;

% Allocate data structures
multiSetup(scenario);

% Locations of radar units
elevs = -6000*ones(1,18);
rdist = 1852*(2:7);
xpos = reshape(repmat([-sind(30); 1; -sind(30)], 1, 1).*rdist, 1, []);
ypos = reshape(repmat([-cosd(30); 0;  cosd(30)], 1, 1).*rdist, 1, []);
scenario.multi.radar_pos = [xpos; ypos; elevs];

scenario.multi.radar_dir = [repmat([60, 180 -60], 1, 6); zeros(1,18)];
...
scenario.traj = struct( ...
    ...
    'time',     0 : 100e-6 : 10*10.24);     % Time of simulation in seconds
...
scenario.simsetup = struct( ...
    ...
    't_p',      2.6e-6, ...             % Pulse duration in seconds
