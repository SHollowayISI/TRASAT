...
scenario.multi.n_fr = 25;
scenario.multi.n_re = 24;
...
zpos = zeros(1,24);
rdist = 1852*(1:8);
xpos = reshape(repmat([-sind(30); 1; -sind(30)], 1, 1).*rdist, 1, []);
ypos = reshape(repmat([-cosd(30); 0; cosd(30)], 1, 1).*rdist, 1, []);
scenario.multi.radar_pos = [xpos; ypos; zpos];

% Normal direction of radar units
scenario.multi.radar_dir = zeros(2,24);
dir_tri = [60, 180, -60];
scenario.multi.radar_dir(1,:) = repmat(dir_tri,1,8);
...
scenario.traj = struct( ...
    ...
    ...
    'pos_st',   [0; 0; 6000], ...          % Position input if 'static' is used
    'vel_st',   [0; 0; 0], ...          % Velocity input if 'static' is used
    ...
    ... % Model options
    'model',    'static', ...               % Set 'static' or 'model'
    'time',     0 : 100e-6 : 20*10.24);     % Time of simulation in seconds
...
scenario.simsetup = struct( ...
    ...
    'thresh',       13, ...             % Detection threshold above noise power in dB
