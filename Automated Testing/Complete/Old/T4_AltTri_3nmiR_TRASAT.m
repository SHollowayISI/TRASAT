...
scenario.multi.n_re = 15;

% Allocate data structures
multiSetup(scenario);

% Locations of radar units
zpos = -200*reshape(ones(3,1).*(0:4), 1, []);
rdist = 1852*3;
xpos = reshape(repmat([-sind(30); 1; -sind(30)], 5, 1).*rdist, 1, []);
ypos = reshape(repmat([-cosd(30); 0; cosd(30)], 5, 1).*rdist, 1, []);
scenario.multi.radar_pos = [xpos; ypos; zpos];

% Normal direction of radar units
scenario.multi.radar_dir = zeros(2,15);
dir_tri = [60, 180, -60];
scenario.multi.radar_dir(1,:) = repmat(dir_tri,1,5);
